...
impl Collector {
    fn metric_type_and_name(&self, m: &data::Metric) -> Option<(MetricType, Cow<'static, str>)> {
        ...
        macro_rules! extract_metric_data_type_and_name {
            ($data:expr, $counter:tt) => {
                match $data {
                    data::MetricData::Gauge(_) => Some((MetricType::GAUGE, name)),
                    ...
                    data::MetricData::Histogram(_) => Some((MetricType::HISTOGRAM, name)),
        ...
        match m.data() {
            data::AggregatedMetrics::F64(data) => extract_metric_data_type_and_name!(data, self),
            data::AggregatedMetrics::U64(data) => extract_metric_data_type_and_name!(data, self),
            data::AggregatedMetrics::I64(data) => extract_metric_data_type_and_name!(data, self),
        }
    }

    fn get_name(&self, m: &data::Metric) -> Cow<'static, str> {
        let name = utils::sanitize_name(&m.name().to_string().into());
...
impl prometheus::core::Collector for Collector {
    ...
    fn collect(&self) -> Vec<MetricFamily> {
        ...
        let mut res = Vec::with_capacity(metrics.scope_metrics().collect::<Vec<_>>().len() + 1);
        ...
        for scope_metrics in metrics.scope_metrics() {
            ...
            for metrics in scope_metrics.metrics() {
                ...
                macro_rules! add_metric {
                    ($data:expr) => {
                        match $data {
                            ...
                            data::MetricData::ExponentialHistogram(_) => {
                                // FIXME(MrCroxx):
                                // add_exponential_histogram_metric(
                                //     &mut res,
                                //     hist,
                                //     description,
                                //     &scope_labels,
                ...
                match metrics.data() {
                    data::AggregatedMetrics::F64(data) => add_metric!(data),
                    data::AggregatedMetrics::U64(data) => add_metric!(data),
                    data::AggregatedMetrics::I64(data) => add_metric!(data),
                }
            }
        }
...
fn add_histogram_metric<T: Numeric>(
    ...
    for dp in histogram.data_points() {
        ...
        let bounds_len = dp.bounds().collect::<Vec<_>>().len();
        let bucket_counts = dp.bucket_counts().collect::<Vec<_>>();
...
// FIXME(MrCroxx):
// fn add_exponential_histogram_metric<T: Numeric>(
//     res: &mut Vec<MetricFamily>,
...
//         let mut h = prometheus::proto::Histogram::default();
//         h.set_sample_sum(dp.sum().as_f64());
//         h.set_sample_count(dp.count() as _);
//         h.set_bucket(bucket);
//         let mut pm = prometheus::proto::Metric::default();
//         pm.set_label(kvs);
//         pm.set_histogram(h);
